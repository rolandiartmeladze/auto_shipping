...
const SlideNavigation = styled.samp`
  ...
  @media (max-width: 500px) {
    bottom: ${(props) => `${-(props.contextheigth + 25)}px`};
    padding-top: 3px;
    padding-bottom: 3px;
    margin-top: 6px;
...
const changeColor = keyframes`
  ...
  33% { color: rgb(200, 50, 0); border-color: rgb(200, 50, 0);}
                    ...
                    height: 6px;
                      ...
                      p {
                        ...
                        line-height: 1.7;
                            ...
                            @media (max-width: 500px) { font-size: 60%; padding: 3px; }


                        }
                        a {
                          color: rgb(255, 255, 0);
                          background-image: linear-gradient(to top, rgb(255, 0, 0, 0.6), rgb(0, 0, 0, 0.6));
                          padding: 8px;
                          margin: 5px;
                          ...
                          box-shadow: 0px 4px 8px rgba(255, 50, 20); 
                        
                          &:hover {
                            color: white !important;
                            background-color: rgba(255, 50, 20, 0.6) ;
                          ...
                          @media (max-width: 1024px) {max-width: 86%; bottom: 40px;}
                          @media (max-width: 700px) {left: 15px; bottom: 30px; max-width: 94%; }

                          @media (max-width: 500px) {left: 0px; 
                            bottom: ${(props) => `${-props.contextheigth}px`};
                            width: 100%;
                            max-width: none;
                          }
`;
...
const Slide = () => {
  ...
  const [contextheigth, setcontextheigth] = useState(0);

  const updateWidths = () => {
    let contextheigth = document.getElementById('context').offsetHeight;
    setcontextheigth(contextheigth)
  ...
  useEffect(() => {
    const interval = setInterval(() => { setSlideIndex((prevIndex) => (prevIndex % slideLength) + 1); }, 9000);
    return () => clearInterval(interval);
  }, [slideLength]);
  const handlePrevSlide = () => { setSlideIndex((slideIndex - 2 + slideLength) % slideLength + 1); };
  const handleNextSlide = () => { setSlideIndex(slideIndex % slideLength + 1); };
  ...
  return (
    <SlideContainer onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
      ...
      <SlideNavigation  contextheigth={contextheigth}>
        {slides.map((_, index) => (
          <SlideNavigationBtn
            key={index}
            style={{
      ...
      <SlideContext contextheigth={contextheigth}  offsetWidth={offsetWidth} >
        <li style={{ zIndex: '15' }}>
